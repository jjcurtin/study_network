...
**COCO**: add in n_spouse, no prop_spouse or prop_prof

no need for n_incoming, n_outgoing, only keep prop

drop raw keep prop in communications (should be the same as in contacts)

report full model (with all features) -> what features matter, remove contacts

report how each model performs
...
features_combined <- features_contacts |> 
...
```{r}
features <- list(features_contacts, features_communications, features_combined)
feature_versions <- c(
                   "features_contacts", "features_communications", 
                   "features_combined",
                   "features_contacts_full", "features_communications_full", 
                   "features_combined_full")
    ...
    data_lst = features |> 
      map(\(feature)
          feature |> 
            ...
            mutate(lapse = as.integer(n_lapse / day_on_study * 91)) |>
            select(-n_lapse, -day_on_study, -subid)),
    feature_versions = feature_versions[1:3],
    outcome_lst = "continuous"
  ) |> expand_grid(
    model_lst = c("poisson", "normal"),
      ...
      map(\(feature)
          feature |> 
            ...
            left_join(baseline |> select(subid, starts_with("demo")), 
                      by = "subid") |>
            select(-n_lapse, -day_on_study, -subid)),
    feature_versions = feature_versions[4:6],
    outcome_lst = "continuous"
  ) |> expand_grid(
    model_lst = c("poisson", "normal"),
    feature_lst = c("raw", "prop", "all")
...
```{r}
err_tbl |>
  write_csv("_results/err_tbl_glmnet_v4.csv")

fits_lst |>
  write_rds(here::here(path_models, "fits_lst_glmnet_v4.rds"))

best_models |>
  write_rds(here::here(path_models, "best_models_glmnet_v4.rds"))
```

```{r}
results <- features_combined |> 
  left_join(labels, by = "subid") |>
  mutate(lapse = as.integer(n_lapse / day_on_study * 91)) |>
  select(lapse,
         prop_friend, prop_drink_presence, prop_support,
         prop_incoming_counselor) |> 
  run_glmnet(outcome = "continuous", model = "poisson", 
             feature_set = "prop")
```
