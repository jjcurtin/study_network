---
title: "GLMNET Models with Contact Network Features"
author: "Coco Yu"
...
# poisson model
update_results(run_glmnet(data = data, outcome = "continuous", 
...
update_results(run_glmnet(data = data, outcome = "binary_high_low", 
...
```{r}
# err_tbl |>
#   write_csv("_results/err_tbl_v2.csv")
# 
# fits_lst |>
#   write_rds(here::here(path_models, "fits_lst_v2.rds"))

best_models |> 
  write_rds(here::here(path_models, "best_models_v2.rds"))
```
...
```{r}
map_dbl(1:30, ~ splits$splits[[.x]] |> analysis() |> pull(lapse) |>  sd()) |> 
  as_tibble() |> print_kbl()

map_dbl(1:30, ~ splits$splits[[.x]] |> assessment() |> pull(lapse) |>  sd()) |> 
  as_tibble() |> print_kbl()
```

```{r}
fits_glmnet <- poisson_reg(penalty = tune(), mixture = tune()) |>
...
```{r}
data_trn <- splits$splits[[1]] |> analysis()
data_test <- splits$splits[[1]] |> assessment()
...
feat_trn <- rec |> prep(data_trn) |> bake(new_data = NULL)
feat_test <- rec |> prep(data_trn) |> bake(new_data = data_test)

fit <- poisson_reg(penalty = 10.49, mixture = 0.4) |>
  set_engine("glmnet", maxit = 1000000) |> 
  fit(lapse ~ ., data = feat_trn)

fit |> tidy()

preds <- predict(fit, new_data = feat_test) |>
  bind_cols(feat_test |> select(lapse))
