...
```{r}
features_msgs <- read_csv(here::here(path_network, "features_network_v6.csv"))
...
configs <- configs |> bind_rows(
  tibble(
    ...
    feature_versions = feature_versions[1],
      ...
      map(\(feature)
          feature |> 
            ...
            left_join(baseline |> select(subid, starts_with("demo")), 
                      by = "subid") |>
            select(-n_lapse, -day_on_study, -subid)),
    feature_versions = feature_versions[2],
    outcome_lst = "continuous"
  ) |> expand_grid(
    model_lst = c("poisson", "normal"),
    feature_lst = c("raw", "prop", "all")
...
configs <- configs |> bind_rows(
  tibble(
    data_lst = features_lst_contact |> 
      map(\(feature)
          feature |> 
            left_join(labels, by = "subid") |>
            left_join(features_msgs, by = "subid") |> 
            mutate(lapse = as.integer(n_lapse / day_on_study * 91)) |>
            select(-n_lapse, -day_on_study, -subid)),
    feature_versions = c("features_msgs_v2_contact_v1", "features_msgs_v2_contact_v2",
    ...
    data_lst = features_lst_contact |> 
      map(\(feature)
          feature |> 
            ...
            mutate(lapse = as.integer(n_lapse / day_on_study * 91)) |>
            select(-n_lapse, -day_on_study, -subid)),
    feature_versions = c("features_msgs_v2_contact_full_v1",
                         "features_msgs_v2_contact_full_v2",
                         "features_msgs_v2_contact_full_v3", 
                         "features_msgs_v2_contact_full_v4"),
    outcome_lst = "continuous"
  ) |> expand_grid(
    model_lst = c("poisson", "normal"),
    feature_lst = c("raw", "prop", "all")
